...
def deduction(input_values):
    ...
    positions["tt"] = list(
        set(digits[7]["bad_segments"]).difference(set(digits[1]["bad_segments"]))
    )[0]
    ...
    for six_segment_input_value in six_segment_input_values:
        if set(digits[1]["bad_segments"]).intersection(six_segment_input_value) != set(
            digits[1]["bad_segments"]
        ):
    ...
    for five_segment_input_value in five_segment_input_values:
        if set(digits[1]["bad_segments"]).intersection(five_segment_input_value) == set(
            digits[1]["bad_segments"]
        ):
    ...
    for six_segment_input_value in six_segment_input_values:
        if set(digits[4]["bad_segments"]).intersection(six_segment_input_value) == set(
            digits[4]["bad_segments"]
        ):
            digits[9]["bad_segments"] = six_segment_input_value

    # figure out 0 since it is the only 6 segment left
    for six_segment_input_value in six_segment_input_values:
        if six_segment_input_value not in [
            digits[6]["bad_segments"],
            digits[9]["bad_segments"],
        ]:
            digits[0]["bad_segments"] = six_segment_input_value

    # use 0 and 8 to figure out what is at position m
    positions["m"] = list(
        set(digits[8]["bad_segments"]).difference(set(digits[0]["bad_segments"]))
    )[0]

    # use 1, 4 and what is at position m to figure out what is at position tl
    positions["tl"] = list(
        set(digits[4]["bad_segments"]).difference(set(digits[1]["bad_segments"]))
    )
    ...
    def solve_part_a(self):
        ...
        for output_values in outputs:
            for output_value in output_values:
                for unique_segment_count_digit in unique_segment_count_digits:
                    digit_segment_count = len(
                        digits[unique_segment_count_digit]["segments"]
                    )
