...
class TicketTrans:
    def __init__(self, input_lines):
        groups = [
            list(group) for k, group in groupby(input_lines, lambda x: x == "") if not k
        ]
        ...
        for nearby_ticket_line in groups[2][1:]:
            self.nearby_tickets.append(
                [int(value) for value in nearby_ticket_line.split(",")]
            )
    ...
    def nearby_tickets_validity(self):
        ...
        for nearby_ticket in self.nearby_tickets:
            (
                are_all_values_in_any_rule,
                invalid_value,
            ) = TicketTrans.all_values_in_any_rule(nearby_ticket, self.rules_list)
            if are_all_values_in_any_rule:
                valid_tickets.append(nearby_ticket)
            else:
                invalid_tickets.append(
                    {"ticket": nearby_ticket, "invalid_value": invalid_value}
                )
