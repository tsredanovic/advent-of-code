...
class Tile:
    ...
    def borders(self):
        border_directions = [Tile.BORDER_TOP, Tile.BORDER_BOTTOM, Tile.BORDER_LEFT, Tile.BORDER_RIGHT]
        borders = {}
...
class Connection:
    def __init__(self, tile1_id, tile2_id, tile1_border, tile2_border, inverted) -> None:
...
def find_connections(tiles):
            ...
            for tile1_border_direction, tile1_border in tile1.borders().items():
                for tile2_border_direction, tile2_border in tile2.borders().items():
                    ...
                    if real_tile1_border == real_tile2_border:
                        conn_touple = (real_tile1.id, real_tile2.id, real_tile1_border, real_tile2_border, False)
                    elif tile1_border[::-1] == tile2_border:
                        conn_touple = (real_tile1.id, real_tile2.id, real_tile1_border, real_tile2_border, True)
    ...
    for conn_touple in conn_touples:
        connections.append(Connection(conn_touple[0], conn_touple[1], conn_touple[2], conn_touple[3], conn_touple[4]))
...
class Y2020D20Solver(BaseSolver):
    ...
    def solve_part_b(self):
        ...
        connections = find_connections(tiles)
        for connection in connections:
            tiles_dict[connection.tile1_id].connects_to.append(tiles_dict[connection.tile2_id])
            tiles_dict[connection.tile2_id].connects_to.append(tiles_dict[connection.tile1_id])
        ...
        for i in range(1, img.wh):
            if i == 1:
                ...
                connects_to_not_mid = [tile for tile in left_tile.connects_to if tile.tag != Tile.TAG_MID]
                connects_to_not_mid_not_used_up = [tile for tile in connects_to_not_mid if tile.id not in used_up_ids]
        ...
        for i in range(1, img.wh-1):
            for j in range(1, img.wh-1):
                neighbour_tiles = [img.tiles[i-1][j], img.tiles[i+1][j], img.tiles[i][j-1], img.tiles[i][j+1]]
                neighbour_tiles = [tile for tile in neighbour_tiles if tile]
                neighbour_connects_to_tiles = [set(tile.connects_to) for tile in neighbour_tiles]
                same_neighbours = list(set.intersection(*neighbour_connects_to_tiles))
                same_neighbours_no_used_up = [tile for tile in same_neighbours if tile.id not in used_up_ids]
