...
class Y2021D03Solver(BaseSolver):
    ...
    def solve_part_b(self):
        ...
        for i in range(len(self.lines[0])):
            bit_to_keep = '0' if current_bit_counts[i][0] > current_bit_counts[i][1] else '1'
            new_stack = []
        ...
        current_bit_counts = get_bit_counts(current_stack)
        for i in range(len(self.lines[0])):
            bit_to_keep = '1' if current_bit_counts[i][0] > current_bit_counts[i][1] else '0'
            new_stack = []
            for line in current_stack:
                if line[i] == bit_to_keep:
                    new_stack.append(line)
        ...
        return int(''.join(oxygen_generator_rating), 2) * int(''.join(co2_scrubber_rating), 2)