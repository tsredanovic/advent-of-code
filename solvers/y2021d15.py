...
def too_slow(lines):
    ...
    paths = [
        {
            'positions': [(0, 0)],
            'len': cavern.position_values[(0, 0)]
        }
    ]
    ...
    while True:
        ...
        if last_position[0] + last_position[1] > closest_pos_sum:
            print('{} -> {} | Paths: {}'.format(last_position, cavern.end_position, len(paths)))
        ...
        not_visited_neighbour_positions = cavern.get_not_visited_neighbour_positions(last_position, shortest_path['positions'])
        for not_visited_neighbour_position in not_visited_neighbour_positions:
            paths.append({
                'positions': shortest_path['positions'] + [not_visited_neighbour_position],
                'len': shortest_path['len'] + cavern.position_values[not_visited_neighbour_position]
            })
...
def still_too_slow(lines):
    ...
    paths = {
        cavern.position_values[(0, 0)]: [[(0, 0)]]
    }
    ...
    while True:
        ...
        not_visited_neighbour_positions = cavern.get_not_visited_neighbour_positions(last_position, shortest_path)
        for not_visited_neighbour_position in not_visited_neighbour_positions:
            new_path_len = shortest_len + cavern.position_values[not_visited_neighbour_position]
            if new_path_len not in paths.keys():
                paths[new_path_len] = [shortest_path + [not_visited_neighbour_position]]
            else:
                paths[new_path_len].append(shortest_path + [not_visited_neighbour_position])
